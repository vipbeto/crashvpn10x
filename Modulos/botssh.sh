...
fun_botOnOff() {
    [[ $(ps x | grep "bot_plus" | grep -v grep | wc -l) = '0' ]] && {
        ...
        fun_bot1() {
            [[ ! -e "/etc/CrashVPN/ShellBot.sh" ]] && wget https://raw.githubusercontent.com/vipbeto/crashvpn10x/main/ShellBot.sh -O /etc/CrashVPN/ShellBot.sh
            cd /etc/CrashVPN
			rm -R bot
			wget https://raw.githubusercontent.com/vipbeto/crashvpn10x/main/Modulos/bot
			chmod 777 bot
